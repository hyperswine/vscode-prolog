...
setup_file_lines_chars(File) :-
	retractall(
		file_lines_computed(File)),
	retractall(
		file_line_start_end(File, _, _, _)),
	read_file_to_string(File, String,
		[encoding(utf8)]),
	atomic_list_concat(List, '\n', String),
	maplist(string_length, List, Lengths1),
	maplist(
		plus(1), Lengths1, [FirstLen|Lengths]),
	assertz(
		file_line_start_end(File, 1, 0, FirstLen)),
...
assert_all_lines(File, [H|T], N) :-
	Last is N - 1,
	file_line_start_end(File, Last, _, LastEnd), ThisStart is LastEnd, ThisEnd is ThisStart + H,
	assertz(
		file_line_start_end(File, N, ThisStart, ThisEnd)), Next is N + 1,
	assert_all_lines(File, T, Next).
assert_all_lines(_, [], _).

from_char_to_line_char(File, CharA, Line, StartChar) :-
	load_source_file(File),
	file_line_start_end(File, Line, Start, End), CharA >= Start, CharA < End, !, StartChar is CharA - Start.
...
sub_chars(Chars, From, To, [H|T]) :-
	nth0(From, Chars, H), Next is From + 1,
...
assert_subgoal(File, From, To) :-
	...
	assert(
		subterm_pos(Goal1, Line, StartChar)).
...
print_properties(Frame, [H|T]) :-
		...
		prolog_frame_attribute(Frame, H, Value), _, fail),
	writeln(H:Value),
	print_properties(Frame, T), !.
...
trace_parent(Frame) :-
	prolog_frame_attribute(Frame, goal, Goal), !,
	writeln(goal:Goal),
	prolog_stack_frame_property(Frame,
		location(File:Line)), !,
	writeln(file:File;
	line:Line),
	prolog_frame_attribute(Frame, parent, Parent),
