...
load_modules(DocTxt) :-
	...
	setup_call_cleanup(
		open_memory_file(DMFH, read, RStream),
...
read_terms(ReadStream, MFH) :-
	repeat,
		...
		read_term(ReadStream, Term,
			[variable_names(VarsNames1),
			term_position(TPos),
			comments(TermComms),
			character_escapes(true)]), E,
		(
			print_message(error, E),
			halt(1))),
	setup_call_cleanup(
		open_memory_file(MFH, write, MFWS),
		portray_clause(MFWS, Term),
		close(MFWS)),
	setup_call_cleanup(
		open_memory_file(MFH, read, MFRS),
		read_term(MFRS, Term1,
			[variable_names(VarsNames2)]),
		close(MFRS)),
	...
	writeln(':::TERMSEGMENTEND'), Term = end_of_file,
	writeln('::::::ALLOVER').
