...
digout_predicate(File, Module:Pred/Arity) :-
	pre_process(File, Module:Pred/Arity), open(File, read, Stream), setval(locations, []), repeat, read_annotated(Stream, Term, AnnTerm), dig_predicate(Module:Pred/Arity, Term, AnnTerm),
	AnnTerm = annotated_term{type:end_of_file}, getval(locations, Locations1), reverse(Locations1, Locations), printf("references:%w%n", [Locations]),
	close(Stream), !.
...
pre_process(File, Module:Pred/Arity) :-
	...
	(
		get_flag(Pred/Arity, definition_module, Module)
		@CurrMod ->
...
dig_predicate(_:_/Arity, Term, AnnTerm) :-
	 \+ is_list(AnnTerm), Arity > 0,
	 \+ compound(Term), !.

dig_predicate(_:Pred/0, Term, AnnTerm) :-
	 \+ is_list(AnnTerm), Pred \= Term,
	 \+ compound(Term), !.
...
dig_predicate(Module:Pred/Arity,  :-import_fromModule, annotated_term{term:AnnTerm}) :-
	(
		AnnTerm = (: - annotated_term{term:AnnTerm1}),

		AnnTerm1 = (importannotated_term{term:AnnTerm2}),

		AnnTerm2 = (annotated_term{term:Ps}
	fromannotated_term{term:Module}),
		locate_pred(Ps, Pred/Arity, From)
		 ->
				update_locs(From);
				true), !.

dig_predicate(_:Pred/_,  :-import_fromPred, _) :-
	!.

dig_predicate(_:Pred/Arity,  :-export_, annotated_term{term:(:-annotated_term{term:(exportPs1)})}) :-
	(
...
unified(PI, Pred/Arity, From) :-
	PI = annotated_term{
		term:annotated_term{term:Pred,
	(from):From}
		/annotated_term{term:Arity}}, !.

unified(PI, Pred/Arity, From) :-
	PI = annotated_term{term:Pred,
	(from):From}
	/annotated_term{term:Arity}.