...
export default class PrologLinter implements CodeActionProvider {
  ...
  private addUseModule(doc: TextDocument, predicate: string, module: string, uri: Uri, range: Range): Thenable<boolean> {
    let edit = new WorkspaceEdit()
    ...
    lines.forEach(line => {
      if (re.test(doc.lineAt(line).text)) directiveLine = line
    })
  ...
  private parseIssue(issue: string) {
    ...
    if (diag) {
      if (!this.diagnostics[fileName]) this.diagnostics[fileName] = [diag]
      else this.diagnostics[fileName].push(diag)
    }
  }

  private doPlint(textDocument: TextDocument) {
    if (textDocument.languageId != "prolog") return
