...
export default class PrologTerminal {
  ...
  private static createPrologTerm() {
    ...
    if (section) {
      ...
      PrologTerminal._terminal = (<any>window).createTerminal(title, executable, args)
    } else throw new Error("configuration settings error: prolog")
  }
  ...
  public static loadDocument() {
    ...
    let goals = `['${fname}']`
    if (PrologTerminal._document.isDirty) PrologTerminal._document.save().then(_ => PrologTerminal.sendString(goals))
    else PrologTerminal.sendString(goals)
  }
  ...
  public static queryGoalUnderCursor() {
    ...
    if (goal.indexOf(":") > -1) goal = goal.split(":")[1]
